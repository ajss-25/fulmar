...
**Decision for build 156 on 2026-09-09: candidate for an explicitly labelled public
...
| Clean checkout reconstruction | Earlier local reconstruction independently reproduced the pinned npm tree. Current bootstrap applies fourteen checksum-bound patches, verifies the 38,501-entry / 394,622,662-byte runtime inventory and prepares the Rust notice-material cache used by the build. The public branch must repeat bootstrap, source gates, and build from its clean clone | Pass for earlier local reconstruction; hosted clean-clone CI on the final source remains required before promoting the draft branch |
| Upstream DSH pin | The exact `0.1.1-rc.1` runtime is reviewed, promotion-provenance-bound and inventory-bound. Current read-only watcher observations and acknowledgements are maintained in `docs/UPSTREAM_DSH_UPGRADES.md`; observing a newer release does not qualify it | Keep `0.1.1-rc.1` for this candidate. Every newer release requires its own cumulative exact-cohort privacy/runtime/migration/provider/tool/rollback qualification and a whole new Fulmar release, never a package-only or installed-runtime update |
...
| Public contribution operations | Security, support, contribution guidance, issue forms, and a pull-request checklist exist. Private vulnerability reporting, secret scanning/push protection, Dependabot, GitHub-owned SHA-pinned Actions, protected `main` with four workflow contexts plus the separate `CodeQL` app check, and immutable `v*` tags are configured | Before release, require all five exact hosted results and retain final app-ID/source-revision binding, history rescans, collaborator/app/webhook review and settings evidence |
...
| Third-party binary obligations | A downloadable app includes `@img/sharp-libvips-darwin-arm64`/libvips binary material whose LGPL/GPL notice, corresponding-source and relinking obligations must be satisfied for the exact shipped payload | **NO-GO.** Exact notices for all 29 upstream component entries are bound, and Rust notice materials are verified through the bootstrap cache and generated notices. This does not close the remaining Rust notice/provenance questions, corresponding-source delivery/offer, relinking/installation-information under code signing or legal clearance. See `docs/LIBVIPS_CORRESPONDING_SOURCE.md`; the `thirdPartyBinaryLicenseMaterials` beta gate remains open pending that evidence and human review |
...
Actions only, protected `main` with the four source-workflow contexts and the separate
`CodeQL` app check required, and immutable `v*` tags. Retain screenshots or an exported settings record before source
...
held. This is the **stable** profile and the default for every public-release command.
A separately identifiable manual-install **beta** profile with its own evidence file
