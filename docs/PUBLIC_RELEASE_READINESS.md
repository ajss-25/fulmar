...
| Upstream DSH pin | The exact `0.1.1-rc.1` runtime is reviewed, promotion-provenance-bound and inventory-bound. The read-only watcher currently observes npm `latest`/`next` at `0.1.1-rc.2` and `alpha` at `0.1.2-alpha.5`; official alpha.5 release 381145530 now binds tag `dsh-v0.1.2-alpha.5` to commit `db6bdc3576c2d4e7c965e8e3ed0c2a731eed87f5` and fixes upgrade startup/session-title loss from rc.2 or alpha.3, but it has not completed an exact-cohort assessment. The corrected `0.1.2-alpha.3` assessment verified an exact 215-package first-party cohort and registry signature, but still found the changed guarded-MCP peer contract and upstream request-identifier behavior Fulmar removes; no later alpha has been promoted or silently assessed as equivalent. No installed or vendored files were changed | Keep `0.1.1-rc.1` for this candidate. Alpha.5 additionally requires retained-state migrations from both affected source versions plus startup and session-title integrity; every newer tag requires its own cumulative exact-cohort privacy/runtime/migration/provider/tool/rollback qualification and a whole new Fulmar release, never a package-only or installed-runtime update |
| CI | Workflow permissions are read-only by default; checkout credentials are not persisted; every GitHub-owned action is exact-SHA-pinned. The first-index gate runs before downloaded code. The source implements exact hosted image/Xcode/SDK/toolchain admission, hash-locked Python/Semgrep closure, two-root unsigned reproducibility, content-bound artifact transport, an exact macOS 15 consumer, and JavaScript/TypeScript-only CodeQL. See `docs/CI_SECURITY.md` | Implementation and focused tests exist. **Hosted-CI observation remains open** until the discovery proposal is reviewed into an active source pin, all four required `Verify source` jobs pass, and `Check upstream DSH` passes on the real repository. CodeQL does not scan Swift |
...
| Public contribution operations | Security, support, contribution guidance, issue forms, and a pull-request checklist exist. Private vulnerability reporting, secret scanning/push protection, Dependabot, GitHub-owned SHA-pinned Actions, protected `main` with four required contexts, and immutable `v*` tags are configured | Actual hosted check results, final app-ID/source-revision binding, final-history rescan, collaborator/app/webhook review and retained settings evidence remain pending |
