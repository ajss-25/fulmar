...
| Disposable funded DeepSeek account (supplementary live-cloud acceptance, section 7) | `[Owner]` | Needed to substantiate a successful live-cloud route, not one of the ten mandatory beta gate records. Use a key created for this exercise with a small balance, revoked afterwards. Never a production key, never a key seen in earlier conversation text or logs. |
