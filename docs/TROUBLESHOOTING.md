...
| `make build` asks for a Keychain password or a Developer ID | Signing variables inherited from your shell | For a preview build run with `LOCAL_HARNESS_NOTARY_PROFILE`, `LOCAL_HARNESS_SIGNING_KEYCHAIN` and `LOCAL_HARNESS_SIGNING_KEYCHAIN_PASSWORD` unset and `LOCAL_HARNESS_REQUIRE_STABLE_SIGNING=0 LOCAL_HARNESS_SIGN_IDENTITY=- LOCAL_HARNESS_SIGN_TIMESTAMP=0` |
...
| --- | --- | --- |
| macOS says the app "cannot be opened", "is damaged", or that Apple could not verify it | The preview is only ad-hoc signed; there is no notarisation ticket | Read [Preview binary and Gatekeeper](PREVIEW_BINARY_GATEKEEPER.md). Prefer building it yourself. Never run `xattr -d`, `spctl --master-disable` or download a copy from an unofficial mirror |
