...
function exactFullRecords(candidate) {
  ...
  while (descriptors.length < 981) descriptors.push({
    file: fixtureFile, name: `synthetic exact lifecycle ${descriptors.length}`
  });
  assert.equal(descriptors.length, 981);
  for (let index = descriptors.length - 278; index < descriptors.length; index += 1) {
  ...
  events.push(
    { type: "test:plan", nesting: 0, count: 703 },
    { type: "test:summary", success: true,
      counts: { tests: 981, passed: 981 - skipped, failed: 0, cancelled: 0, skipped, todo: 0 } }
  );
  return events;
}
...
test("JavaScript event verifier rejects unsafe metadata and a partial full-suite topology", async () => {
  ...
  for (const [profile, candidate] of [["full-source", false], ["full-candidate", true]]) {
    ...
    assert.equal(reported.length, 1964);
